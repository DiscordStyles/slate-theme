...
export function createRollupConfigs(config) {
  ...
  return [!isNollup && baseConfig(config, { dynamicImports: false }), useDynamicImports && baseConfig(config, { dynamicImports: true }), !isNollup && serviceWorkerConfig(config)].filter(Boolean);
}
...
function baseConfig(config, ctx) {
  ...
  const outputConfig = !!dynamicImports ? { format: "esm", dir: buildDir } : { format: "iife", file: `${buildDir}/bundle.js` };
  ...
  function transform(contents) {
    const scriptTag = typeof config.scriptTag != "undefined" ? config.scriptTag : '<script type="module" defer src="/build/main.js"></script>';
...
function serviceWorkerConfig(config) {
  ...
  const _rollupConfig = {
    ...
    plugins: [commonjs(), resolve({ browser: true }), production && terser(), replace({ "process.env.NODE_ENV": "'production'" })],
  };
